                            ...
                            <template v-for="item in getTablesData()">
                                <lb-table-column prop="data" label="教师">
                                    <template scope="scope">
                                        <a class="link ng-binding" ng-click="params.oe_id=item.oe_id;params_teacher_name=item.teacher_name">{{item.teacher}}</a>
                                    </template>
                                </lb-table-column>
                                <lb-table-column prop="data" label="授课时间">
                                    <template scope="scope">g</template>
                                </lb-table-column>
                                <lb-table-column prop="data" label="时长">
                                    <template scope="scope">
                                        <span class="badge bg-success ng-binding">{{item.hour}}</span>
                                    </template>
                                </lb-table-column>
                                <lb-table-column prop="data" label="学生数">
                                    <template scope="scope">{{item.hour}}</template>
                                </lb-table-column>
                                <lb-table-column prop="data" label="类型">
                                    <template scope="scope">{{item.type}}</template>
                                </lb-table-column>
                                <lb-table-column prop="data" label="对象">
                                    <template scope="scope">{{item.object}}</template>
                                </lb-table-column>
                                <lb-table-column prop="data" label="课程金额">
                                    <template scope="scope">
                                        <a ng-click="$util.open('tpl/app/hours/view_lesson_amount.modal.html','md',item)" @click="lbShowdialog($event,'lb-viewlessonamountmodal')">
                                            <span class="label bg-info ng-binding">{{item.money}}</span>
                                        </a>
                                    </template>
                                </lb-table-column>
                                <lb-table-column prop="data" label="点评">
                                    <template scope="scope">
                                        <span ng-if="item.is_comment == '0'" class="badge bg-warning ng-scope">{{item.comment}}</span>
                                    </template>
                                </lb-table-column>
                            </template>
                        </lb-table>
