        ...
        <div class="modal-content" modal-transclude="">
            ...
            <div ng-controller="remove_class_attendance" class="ng-scope">
                <div class="modal-header">
                    ...
                    <h3 class="modal-title"><i class="fa fa-calendar"></i>撤销 <span class="label bg-info ng-binding">11</span> 的考勤记录</h3></div>
                <div class="modal-body">
                    <div class="row no-gutter step1" ng-class="{'step1':step==1,'step2':step==2}">
                        <div class="col-xs-6 bg-light lter bg-success" ng-class="{'bg-success':step==1}">
                            <h4 class="padder">1.选择考勤记录</h4></div>
                        <div class="col-xs-6 bg-light lter" ng-class="{'bg-success':step==2}">
                            <h4 class="padder">2.确认撤销</h4></div>
                    </div>
                    <!-- ngIf: step==1 -->
                    <div class="m-t ng-scope" ng-if="step==1">
                        <div class="row">
                            <!-- ngRepeat: item in attendances_rest.$list -->
                        </div>
                        <!-- ngIf: attendances_rest.$loading -->
                        <!-- ngIf: attendances_rest.$loaded && attendances_rest.$list.length == 0 -->
                        <p class="text-center text-muted ng-scope" ng-if="attendances_rest.$loaded &amp;&amp; attendances_rest.$list.length == 0">没有可供撤销的已考勤记录!</p>
                        <!-- end ngIf: attendances_rest.$loaded && attendances_rest.$list.length == 0 -->
                    </div>
                    <!-- end ngIf: step==1 -->
                    <!-- ngIf: step==2 -->
                </div>
                <div class="modal-footer"><a ng-show="step > 1" class="btn btn-link ng-hide" ng-click="go_step(1)">返回</a>
                    ...
                    <button class="btn btn-primary ng-hide" ng-show="step == 2" ng-click="remove_attendance()">
                        <!-- ngIf: saving -->确认撤销</button>
