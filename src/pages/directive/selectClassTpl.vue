    ...
    <div class="modal-dialog" ng-class="{'modal-sm': size == 'sm', 'modal-lg': size == 'lg','modal-full':size == 'full'}">
        <div class="modal-content" modal-transclude="">
            <div class="modal-header ng-scope">
                ...
                <h3 class="modal-title ng-binding"><i class="fa fa-flag"></i> 请选择班级<!-- ngIf: vm.maxNum > 1 --></h3></div>
            <div class="modal-body ng-scope">
                ...
                filter.fields = [
                  {name:'class_name',value:'班级名称'},
                  {name:'master',value:'老师姓名'}
                ];
                grid.search_key = 'class_name';
                grid.search_value = '';
                ">
                                        <button type="button" class="btn btn-default btn-sm ng-pristine ng-untouched ng-valid" ng-model="grid.search_key" data-html="1" bs-options="item.name as item.value for item in filter.fields" bs-select="">班级名称&nbsp;<span class="caret"></span></button>
                                    </div>
                        ...
                        <div class="col-xs-12 col-md-5"><a ui-per="class.add" class="btn btn-primary btn-sm" ng-click="$util.open('tpl/app/lesson/classes/open_class.modal.html','md')"><i class="fa fa-plus"></i>新班级</a></div>
                    </div>
                    <!--row-->
                    <ul class="list-group m-t">
                        ...
                        <li class="list-group-item ng-scope" ng-class="{'active':vm.is_selected(item)}" ng-click="vm.select(item)" ng-repeat="item in grid.data" ng-if="!loading">
                            <h4 class="list-group-item-heading ng-binding">3期班 <!-- ngIf: item.student_count >= item.max_student_num --></h4>
                        ...
                        <!-- ngIf: !loading -->
                        <li class="list-group-item ng-scope" ng-class="{'active':vm.is_selected(item)}" ng-click="vm.select(item)" ng-repeat="item in grid.data" ng-if="!loading">
                            <h4 class="list-group-item-heading ng-binding">1班 <!-- ngIf: item.student_count >= item.max_student_num --></h4>
                        ...
                        <!-- ngIf: !loading -->
                        <li class="list-group-item ng-scope" ng-class="{'active':vm.is_selected(item)}" ng-click="vm.select(item)" ng-repeat="item in grid.data" ng-if="!loading">
                            <h4 class="list-group-item-heading ng-binding">13414 <!-- ngIf: item.student_count >= item.max_student_num --></h4>
            ...
            <script type="text/ng-template" id="select_class_tpl.ctrl.js" class="ng-scope">$scope.$on('classes:refresh',function(){ $scope.rest_get(); });
            </script>
        </div>
    </div>
</template>
