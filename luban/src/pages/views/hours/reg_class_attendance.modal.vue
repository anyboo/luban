<template>
    <div class="modal-dialog modal-lg" ng-class="{'modal-sm': size == 'sm', 'modal-lg': size == 'lg','modal-full':size == 'full'}">
        <div class="modal-content" modal-transclude>
            <div oc-lazy-load="['js/modules/hours.js']" ></div>
            <div ng-controller="reg_class_attendance" >
                <div class="modal-header">
                    ...
                    <h3 class="modal-title">
                        ...
                        <span class="label bg-info">11</span>的考勤
                    </h3>
                </div>
                <div class="modal-body">
                    <div class="row no-gutter step1" ng-class="{'step1':step==1,'step2':step==2}">
                        <div class="col-xs-6 bg-light lter bg-success" ng-class="{'bg-success':step==1}">
                            <h4 class="padder">1.选择排课</h4>
                        </div>
                        <div class="col-xs-6 bg-light lter" ng-class="{'bg-success':step==2}">
                            <h4 class="padder">2.登记考勤</h4>
                        </div>
                    </div>
                    <div class="m-t" ng-if="step==1">
                        <div class="row no-gutter"></div>
                        <p class="text-center text-muted" ng-if="arranges_rest.$loaded && arranges_rest.$list.length == 0">
                            没有排课记录，请先进行
                            <a ng-click="$util.open('tpl/app/lesson/classes/lesson_arrange.modal.html','lg',class)" @click="lbShowdialog($event,'lb-lessonarrangemodal')">排课</a>!
                        </p>
                    </div>
                </div>
                <div class="modal-footer">
                    ...
                    <button class="btn btn-primary" ng-show="step == 2" ng-disabled="saving" ng-click="reg_attendance()">登记考勤</button>
