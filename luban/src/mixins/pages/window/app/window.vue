<template>
    <div class="window window-active window-loaded" style="width: 900px; height: 500px; left: 34px; top: 155.5px; z-index: 1003; display: none;" :style="{'border': '1px '+getcolor+' solid'}" :class="{'window-maximize':windowmax}">
        ...
        <!--
        <div class="ui-resizable-handle ui-resizable-n" style="z-index: 90;" @mousedown="handleHeaderDown($event,false)"></div>
        <div class="ui-resizable-handle ui-resizable-e" style="z-index: 90;" @mousedown="handleHeaderDown($event,false)"></div>
        <div class="ui-resizable-handle ui-resizable-s" style="z-index: 90;" @mousedown="handleHeaderDown($event,false)"></div>
        <div class="ui-resizable-handle ui-resizable-w" style="z-index: 90;" @mousedown="handleHeaderDown($event,false)"></div>
        -->
        <div class="ui-resizable-handle ui-resizable-se ui-icon ui-icon-gripsmall-diagonal-se" style="z-index: 90;" @mousedown="handleHeaderDown($event,false)"></div>
       <!-- <div class="ui-resizable-handle ui-resizable-sw" style="z-index: 90;" @mousedown="handleHeaderDown($event,false)"></div>
        <div class="ui-resizable-handle ui-resizable-ne" style="z-index: 90;" @mousedown="handleHeaderDown($event,false)"></div>
        <div class="ui-resizable-handle ui-resizable-nw" style="z-index: 90;" @mousedown="handleHeaderDown($event,false)"></div>
        -->
    </div>
</template>
<style>
        ...
        handleHeaderDown(ev, isdrag) {
            ...
            if (!this.windowmax) {
                ...
                if (isdrag) {
                    ...
                    this.disx = this.$el.offsetLeft || 0
                    this.disy = this.$el.offsetTop || 0
                }
                ev.preventDefault();
                this.addListener(window, "blur", this.handleHeaderUp)
                this.addListener(document, 'mousemove', this.handleHeaderMove)
                this.addListener(document, 'mouseup', this.handleHeaderUp)
            }
        },
        handleHeaderMove(ev) {
            if (this.moveflag && !this.windowmax) {
                ...
                if (this.isdrag) {
                    ...
                    this.$el.style.width = Math.max(i_x, 300) + 'px'
                    this.$el.style.height = Math.max(i_y, 200) + 'px'
                }
            }
        },
        handleHeaderUp(ev) {
            this.removeListener(document, 'mousemove', this.handleHeaderMove)
            this.removeListener(document, 'mouseup', this.handleHeaderUp)
            this.removeListener(window, "blur", this.handleHeaderUp)
            this.moveflag = false
        },
        handleWindowMin() {
